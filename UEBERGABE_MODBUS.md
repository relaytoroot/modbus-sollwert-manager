# Uebergabe FGH Modbus Sollwert Manager

## Projekt
Der **FGH Modbus Sollwert Manager** ist eine Windows-Desktop-Anwendung zur Planung und Ausfuehrung von Modbus-TCP-Testablaeufen. Die Anwendung verbindet sich mit einem Modbus-Geraet, schreibt Sollwerte auf bis zu vier konfigurierbare Kanaele und fuehrt einen Testplan stufenweise aus.

Made by Yunus Sevgi.

Version laut Projektstand:
- Name: `FGH Modbus Sollwert Manager`
- Version: `1.0.0`
- Firma: `FGH`
...
- Frei waehlbare Bezeichnungen pro Sollwertkanal
...
- Umschaltbare helle und dunkle Darstellung
...
Einstiegspunkt:
- `start_modbus_sollwert_manager.py`
...
```text
dist\Release_FGH_Modbus_Sollwert_Manager\FGH_Modbus_Sollwert_Manager.exe
```
...
- erstellt danach den Release-Ordner `dist\Release_FGH_Modbus_Sollwert_Manager`
...
- `modbus_gui/main_window.py`
  Enthaelt das Hauptfenster, die Tabellenlogik, die Bedienung, den Verbindungsablauf, Start/Stopp, Excel Laden/Speichern und die Statusanzeige.
...
- `modbus_gui/app_info.py`
  Enthaelt Name, Version, Beschreibung, Autorhinweis und Logo-Bezug.
...
- `Start_FGH_Modbus_Sollwert_Manager.bat`
- benoetigte Laufzeitdateien im Unterordner `_internal`

## Hinweise fuer die Abgabe
Empfohlen zur Weitergabe:
1. Den kompletten Ordner `dist\Release_FGH_Modbus_Sollwert_Manager`
